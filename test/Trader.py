...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        self.sig = Sig() # 시그널 객체 생성

    def signal_(self): # 시그널 신호
        self.sig.signal()
    ...
    # 실시간 등록
    def set_realreg(self, scrno, trcode, fid, realtype):
        self.dynamicCall("SetRealReg(QString, QString, QString, QString)", scrno, trcode, fid, realtype)
    ...
    def _receive_real_data(self, scode, realtype, realdata):
        if realtype == "주식체결":
            ...
            self.prcie = int(self.GetCommRealData(scode, 10))
            if self.prcie < 0:
            ...
            self.cjcode = self.get_master_code_name(scode)
        self.Item.item_view[scode] = [self.cjcode, str(self.price), self.price_up_down]
        print("item" + Item.item_view)
        self.sig.sinal()
